...
<div class="flex items-center justify-center min-h-[calc(100vh-10rem)]">
    <div class="w-full max-w-2xl">
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold text-slate-800">Tambah Departemen Baru</h2>
                <p class="text-sm text-slate-500 mt-1">Masukkan informasi departemen yang akan ditambahkan.</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <form action="{{ route('management.departments.store') }}" method="POST">
                @csrf
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="block w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2.5 border placeholder-slate-400"
                            placeholder="Deskripsi singkat mengenai departemen ini (opsional)">{{ old('description') }}</textarea>
                        @error('description')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
