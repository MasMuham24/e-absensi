...
<div class="flex items-center justify-center min-h-[calc(100vh-10rem)]">
    <div class="w-full max-w-2xl">
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h2 class="text-xl font-semibold text-slate-800">Edit Kantor</h2>
                <p class="text-sm text-slate-500 mt-1">Ubah informasi kantor.</p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
            <form action="{{ route('management.offices.update', ['office' => $office->id]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="latitude" id="latitude" value="{{ old('latitude', $office->latitude) }}" required
                            class="block w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2.5 border placeholder-slate-400"
                    ...
                    <div>
                        ...
                        <input type="text" name="longitude" id="longitude" value="{{ old('longitude', $office->longitude) }}" required
                            class="block w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2.5 border placeholder-slate-400"
                    ...
                    <div>
                        ...
                        <input type="number" name="radius" id="radius" value="{{ old('radius', $office->radius) }}" required min="10" max="5000"
                            class="block w-full border-slate-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-4 py-2.5 border placeholder-slate-400"
