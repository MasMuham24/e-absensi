...
@section('header_title', 'Detail Pengajuan Cuti')
        ...
        <div class="space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <p class="text-sm font-medium text-slate-800">{{ $leaveRequest->user->name ?? '-' }}</p>
                    <p class="text-xs text-slate-500">{{ $leaveRequest->user->employee_code ?? '-' }}</p>
                </div>

                <div>
                    <h3 class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Jenis Cuti</h3>
                    @if($leaveRequest->leave_type === 'cuti')
                        <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-indigo-100 text-indigo-800">Cuti</span>
                    @elseif($leaveRequest->leave_type === 'sakit')
                        <span class="px-2.5 py-1 text-xs font-semibold rounded-full bg-rose-100 text-rose-800">Sakit</span>
                    @elseif($leaveRequest->leave_type === 'penting')
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-800">{{ \Carbon\Carbon::parse($leaveRequest->start_date)->translatedFormat('d M Y') }} s/d {{ \Carbon\Carbon::parse($leaveRequest->end_date)->translatedFormat('d M Y') }}</p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm text-slate-800">{{ $leaveRequest->approver->name ?? '-' }}</p>
                </div>
            </div>

            <div>
                <h3 class="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-2">Alasan</h3>
                <p class="text-sm text-slate-600 whitespace-pre-wrap">{{ $leaveRequest->reason }}</p>
            </div>

            <div class="flex justify-end space-x-3">
                <a href="{{ route('hr.leave-requests.index') }}" class="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Kembali
                </a>
            </div>
        </div>
